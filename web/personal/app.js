...
const views={candidate:'待确认记忆',active:'当前记忆',archived:'已归档',profile:'个人偏好',agents:'已登记 Agent',jobs:'整理任务'};
let sourceId='',token='',view='candidate',offset=0,nextOffset=null,current=null,editing=null,pending=null,busy=false,loadVersion=0;
...
async function submitPending(){
  ...
  try{
    if(['commit','capture'].includes(pending.operation))await api('register',{agent_id:'personal-console',agent_type:'general_agent',capabilities:[]});
...
function mutate(operation,input){if(busy||pending){message('请先处理尚未确认的请求。',true);return;}pending={operation,input:{...input,...(['commit','capture','update','review'].includes(operation)?{event_id:crypto.randomUUID()}: {})}};submitPending();}
...
async function load(){
  ...
  $('view-title').textContent=views[view];$('search-form').hidden=['profile','agents','jobs'].includes(view);
  try{const data=await api(view==='jobs'?'jobs':view==='agents'?'agents':view==='profile'?'profile':'search',['agents','jobs'].includes(view)?{limit:20,offset}:view==='profile'?{limit:50,budget_bytes:131072}:{status:view,query:$('query').value,limit:20,offset,budget_bytes:131072});if(request===loadVersion&&token)render(data);}
