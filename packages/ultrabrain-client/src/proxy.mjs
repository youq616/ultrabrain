...
export async function serveProxy(connection) {
  ...
  server.setRequestHandler(CallToolRequestSchema,async(request,extra)=>{
    ...
    }catch(e){const code=e instanceof UltraError&&/^[a-z0-9_]{1,64}$/.test(e.code)?e.code:'upstream_failed';return {isError:true,content:[{type:'text',text:JSON.stringify({error:code,delivery:submitted?'unconfirmed':'not_submitted'})}]};}
