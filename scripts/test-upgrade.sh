...
BASE=38e0e6657561e17ba85c71d0783d604242a054e8
WORK="$(mktemp -d "${RUNNER_TEMP:-/tmp}/ub-upgrade.XXXXXX")"
# Old executable source is immutable; never infer it from the candidate's schema.
...
# Build new binaries separately, then let the manager reject any unimplemented
# cross-major migration rather than pretending compatibility.
cd "$ROOT"
...
# Only the synthetic report is published; never upload the database, outbox or secrets.
