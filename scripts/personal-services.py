...
def export_plan(value, output, expected):
    ...
    try:
        ...
        write_file(dest, 'INCOMPLETE', b'Incomplete export; do not install.\n')
...
def verify_export(value, directory, expected):
    ...
    try:
        ...
        need(set(os.listdir(fd)) == set(wanted), 'export_inventory_mismatch')
        for name, text in wanted.items():
            ...
            try:
                ...
                need(stat.S_ISREG(before.st_mode) and before.st_nlink == 1 and before.st_uid == os.geteuid()
                     and not before.st_mode & 0o077, 'unsafe_export_file')
                raw = text.encode()
                need(before.st_size == len(raw), 'export_content_mismatch')
                got = bytearray()
                while len(got) <= len(raw):
                    chunk = os.read(file, len(raw)+1-len(got))
                    if not chunk:
                        break
                    got.extend(chunk)
                after = os.fstat(file)
                ...
                need((current.st_dev, current.st_ino) == (before.st_dev, before.st_ino), 'output_path_changed')
            finally:
                os.close(file)
