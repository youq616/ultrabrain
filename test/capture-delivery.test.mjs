...
test('normal capture checks identity and authorization before each write',async()=>{
 ...
 assert.deepEqual(f.calls.map(x=>Array.isArray(x)?x[0]:x),['identity','authorized','ultra_agent_register','identity','authorized','ultra_personal_capture']);
...
test('async authorization is rejected rather than treated as a successful assertion',async()=>{
 const f=fixture();await assert.rejects(deliverCapture(payload(),profile,{...f,authorize:async()=>{}}),{code:'invalid_params'});assert.deepEqual(f.calls,['identity']);
});
