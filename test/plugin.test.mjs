...
test('upstream operation removal fails closed',()=>{
  const f=fixture();f.operations.pop();assert.throws(()=>registerPlugin(f.operations,f.native,async()=>({profile:null})),{code:'upstream_contract_changed'});
});
test('duplicate registration is rejected',()=>{
  const f=fixture();registerPlugin(f.operations,f.native,async()=>({profile:null}));
  assert.throws(()=>registerPlugin(f.operations,f.native,async()=>({profile:null})),{code:'upstream_contract_changed'});
});
...
test('native parameter contract drift stops before handler execution',async()=>{
  const f=fixture();f.native.validateParams=()=> 'shape changed';registerPlugin(f.operations,f.native,async()=>({profile:null}));
