...
queries = {
    ...
    'personal_documents': "SELECT count(*)::text || ':' || coalesce(md5(string_agg(id::text || ':' || label || ':' || format || ':' || encode(content,'hex') || ':' || content_sha256 || ':' || byte_size::text || ':' || has_bom::text || ':' || status || ':' || revision::text, ',' ORDER BY id)),'empty') FROM ultrabrain.personal_documents",
    'personal_document_fragments': "SELECT count(*)::text || ':' || coalesce(md5(string_agg(id::text || ':' || document_id::text || ':' || memory_id::text || ':' || byte_start::text || ':' || byte_end::text || ':' || fragment_sha256 || ':' || offset_unit, ',' ORDER BY id)),'empty') FROM ultrabrain.personal_document_fragments",
