...
try {
  ...
  const request=(operation,more={})=>({operation,consent:true,files:['compare','duplicate-compare'].includes(operation)?[{path},{path:next}]:[{path}],...more});
  ...
  for(const mode of ['cli','library','bytes'])for(const operation of ['inspect','compare','page','record','audit','trace','impact','duplicates','duplicate-compare']){
    ...
    if(operation==='duplicate-compare'){assert.deepEqual(report.result.counts,{groups:1,left_only:1,right_only:0,changed:0,unchanged:0});assert.equal(report.result.groups[0].right.member_count,0);assert.equal(report.result.merge_safe,false);}
