...
export function bridgeOptions(args) {
 const options={capture:false,memoryPolicy:'current'};const seen=new Set();
 for(let n=0;n<args.length;n++) {
  ...
  requireThat(['--url','--token-file','--root','--outbox','--memory-policy','--visibility'].includes(key)&&n+1<args.length,'invalid_params','Unknown or incomplete bridge option');
 ...
 return {
  ...
  async handle(event) {
   ...
   const memory=new AgentMemory({client,rootUri:options.rootUri,sessionId:event.session_id,projectId:event.project_id??null,
    capture:options.capture,deferExtraction:true,visibility:options.visibility,memoryPolicy:options.memoryPolicy,
