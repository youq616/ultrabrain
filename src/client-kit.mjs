...
export function requiredClientTools(profile) {
  return [...new Set([...CLIENT_REQUIRED_TOOLS,...(profile.allowCapture?['ultra_agent_register','ultra_personal_capture']:[]),...(profile.allowDocuments?['ultra_agent_register','ultra_personal_document_import','ultra_personal_document_read','ultra_personal_document_list','ultra_personal_document_queue','ultra_personal_document_archive']:[])])];
}
export function clientProfile(input) {
  objectFields(input,['format','source','project_id','workspace','server','expected_instance','expected_actor','budget_bytes','timeout_ms','allow_capture','outbox_directory','automatic_capture','allow_documents']);
