...
export function snapshotRequest(input) {
  ...
  const choices = {duplicates:['operation','consent'],inspect:['operation','consent'],compare:['operation','consent'],
...
export function snapshotFailure(error) {
  let code;
  // Callback exceptions are untrusted too: never run a code accessor or let a
  // revoked Proxy turn sanitization into an unchecked second exception.
  ...
  try {
    ...
    const report = {format:'ultrabrain-client-snapshot-v1',operation:request.operation,local_only:true,read_only:true,
      ...
      limitations:['unsigned-self-declared-owner','no-live-database-check','absence-is-not-deletion',
        ...
        request.operation === 'duplicates' ? 'same-file-exact-text-not-merge-authority' :
