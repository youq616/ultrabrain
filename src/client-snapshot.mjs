...
import {
  inspectMemorySnapshotFile,compareMemorySnapshots,compareMemorySnapshotDuplicates,queryMemorySnapshot,readMemorySnapshotRecord,
...
export function snapshotRequest(input) {
  ...
  const choices = {inspect:['operation','consent'],compare:['operation','consent'],
    'duplicate-compare':['operation','consent'],duplicates:['operation','consent'],audit:['operation','consent','memory_id'],impact:['operation','consent','memory_id'],trace:['operation','consent','memory_id','max_hops'],page:['operation','consent','options'],record:['operation','consent','memory_id','include_text']};
  ...
  if (request.operation === 'impact') requireThat(uuid(request.memory_id),'invalid_params','A complete impact root ID is required');
  if (request.operation === 'trace') {
    request.max_hops = request.max_hops === undefined ? SNAPSHOT_TRACE_DEFAULT_HOPS : request.max_hops;
    requireThat(uuid(request.memory_id) && validSnapshotTraceHops(request.max_hops),
      'invalid_params','Invalid snapshot trace selection');
  }
...
export async function inspectClientSnapshotBytes(input, inputs, {authorize=()=>{},signal}={}) {
  ...
  try {
    allowed();
    const request = snapshotRequest(input), count = ['compare','duplicate-compare'].includes(request.operation) ? 2 : 1;
    ...
    else if (request.operation === 'duplicate-compare') result = await compareMemorySnapshotDuplicates(files[0],files[1],allowed);
    ...
    const report = {format:'ultrabrain-client-snapshot-v1',operation:request.operation,local_only:true,read_only:true,
      ...
      limitations:['unsigned-self-declared-owner','no-live-database-check','absence-is-not-deletion',
        ...
        ['duplicates','duplicate-compare'].includes(request.operation) ? 'exact-content-match-is-not-a-merge-plan' :
