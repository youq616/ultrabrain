...
export const migrations=['0001-baseline','0002-revision-evidence','0003-deferred-sessions','0004-summary-cache','0005-memory-policy','0006-instance-identity','0007-review-dependencies','0008-fact-evidence','0009-enterprise-admission','0010-personal-memory-core','0012-personal-core-wiring','0013-personal-consolidation','0014-personal-documents'].map(id =>
...
export function validateHistory(rows, plan=migrations) {
  ...
  requireThat(new Set(ids).size===ids.length && ids.every((id,i)=>/^\d{4}-[a-z0-9-]+$/.test(id)&&(i===0||id>ids[i-1])) && plan.every(m=>Array.isArray(m.statements)&&m.statements.length>0&&m.statements.every(s=>typeof s==='string'&&s.trim())), 'migration_invalid','Invalid migration plan');
  requireThat(rows.length<=plan.length,'migration_unknown','Database contains migrations unknown to this release');
  for(let i=0;i<rows.length;i++){requireThat(rows[i].id===plan[i].id,'migration_unknown','Migration history is not ordered');requireThat(rows[i].checksum===checksum(plan[i]),'migration_checksum_mismatch','Migration changed');}
...
export async function applyMigrations(engine,{plan=migrations}={}){
 ...
 return engine.transaction(async tx=>{
  ...
  await tx.executeRaw(`CREATE TABLE IF NOT EXISTS ultrabrain.schema_migrations (id text PRIMARY KEY,checksum text NOT NULL CHECK(length(checksum)=64),applied_at timestamptz NOT NULL DEFAULT now())`);
  ...
  for(const migration of pending){for(const statement of migration.statements) await tx.executeRaw(statement);await tx.executeRaw('INSERT INTO ultrabrain.schema_migrations(id,checksum) VALUES ($1,$2)',[migration.id,checksum(migration)]);}
