...
export async function health() {
  ...
  try {
    ...
    const checks={deferred_schema_present:deferredSchema.present,non_superuser:role.rolsuper===false,no_role_or_database_creation:!role.rolcreatedb&&!role.rolcreaterole,
      ...
      custom_tools_registered:['ultra_read','ultra_ls','ultra_retrieve','ultra_write','ultra_delete','ultra_commit_session','ultra_process_sessions','ultra_session_status','ultra_summarize','ultra_summary_status','ultra_summary_forget','ultra_excerpt',...PROJECT_TOOL_NAMES].every(name=>operations.some(x=>x.name===name))};
