...
采集与文档导入 API、采集队列的可选 `authorize` 回调必须是同步断言：返回 `false` 或抛错表示拒绝，成功可返回 `undefined`（兼容已有调用方式）。返回 Promise 会被拒绝；异步读取授权信息应在调用前完成，并在同步回调中校验仍有效的状态。队列在等待锁后、交付在等待连接/身份校验后复查授权。撤回只阻止后续保存或发送，已经发送且取得匹配回执的事件仍可完成本地清理。
