...
## Separate Codex review of 9b14fde and follow-up

The actual independent GitHub reviewer `chatgpt-codex-connector[bot]` completed review 5220605354 and raised two P2 findings on candidate 9b14fde76666a1115b86f3ab0350e6790bdea039: integration safety opt-in/root guards could be compiled away (comment4024245747), and descriptor-only file signature checks could accept a stale private file after ancestor-directory replacement (comment4024245756). These are separate-reviewer findings, not attributed to this implementation self-review. The prior candidate was not merged.

Eight new regressions failed on the reviewed candidate before fixes. Five use actual optimized Python subprocesses (-O, -OO and PYTHONOPTIMIZE) but intercept the first private read; no real installation, chmod or database process is ever reached. Three use synthetic directory replacement/permission changes during or after reading state. The corrected integration script enforces opt-in and non-root checks unconditionally, then refuses disabled assertions even with consent. The preflight reader rewalks each managed path from the held home descriptor after reading, compares ancestor identities plus file signature, and revalidates observed paths and home permissions before finalizing the report. Rechecks do not reread secret contents or claim an atomic cross-process snapshot.

All eight regressions pass after correction. The full local suites reran with483 JavaScript and176 Python tests passing. Subsequent native/CI/independent-review results must be bound to the new complete candidate, not inferred from approval or tests of9b14fde. No migrations, pins, dependency versions, service behavior or memory permissions were altered by these fixes.
