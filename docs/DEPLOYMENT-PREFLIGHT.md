...
路径读取使用不跟随符号链接的逐级目录描述符；私有文件要求当前账号所属、无组/其他用户权限、普通文件且非硬链接。FIFO、设备或链接不能伪装成配置。配置读取有大小上限，读取中检测变化；读完后从持有的数据目录描述符重新解析路径，核对父目录和文件身份，并在报告结束前再次检查已观察的路径与目录权限，避免只校验被移走的旧文件。固定源码入口也拒绝错误类型和符号链接。所有结果使用固定安全代码，不把解析异常原样返回。
...
在已 bootstrapped 的隔离 Linux CI 中运行 `ULTRABRAIN_TEST_ALLOW_WRITE=1 python3 -B test/preflight-integration.py`，检查真实 Python/Bun CLI、实际托管安装状态、配置指纹、数据库停止后不会被预检启动，以及权限失败不会被自动修复。该测试短暂停止并恢复本次合成安装的数据库，禁止对重要记忆库运行。授权和普通账号检查使用不能被优化掉的运行时判断；测试脚本遇到 `python -O/-OO` 或 `PYTHONOPTIMIZE` 时拒绝执行，以免测试断言被移除后仍操作安装。恢复专项工作流接入安装前检查和这项集成；远程结果必须绑定最终候选提交，未执行不能写成通过。
...
路径重查只能发现检查时仍可观察的变化，不是锁住所有本地进程的原子快照；检查完成后的改变仍须由后续实际操作处理。
