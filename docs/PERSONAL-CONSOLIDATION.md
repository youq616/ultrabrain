...
任务等待数据库准入后、实际调用模型前，会再次核对个人模型仍已启用且 profile 指纹不变；默认网关加载后的调用入口也会复查配置和取消信号。等待期间关闭模型或改变模型/版本配置，任务以 `model_profile_changed` 失败且不向模型发送本次原文。已发出的请求与费用无法撤回；修复配置后仍需明确重试。
