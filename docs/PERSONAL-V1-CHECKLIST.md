...
| 个人日常可用性 | 已有离线 install/runtime 预检、个人服务计划/导出/校验、获准 Worker 周期循环及只读 personal-status。PR #7/#9 已合入服务计划与状态；本阶段增加停止状态下的受管安装、更新、回滚与中断恢复，范围见 PERSONAL-DEPLOY.md，最终验收按新提交的 CI/独立审核记录。自动激活切换与用户主机实际部署仍待完成。预检或单元活动状态通过不代表应用就绪。 |
