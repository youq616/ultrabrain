...
| 个人日常可用性 | 已有离线 install/runtime 预检、个人服务计划/导出/校验、获准 Worker 周期循环及只读 personal-status。PR #7/#9 已合入服务计划与状态，并有一次性 CI 用户 systemd 验证；完整个人服务安装/应用/回滚及用户主机实际部署仍待完成。预检或单元活动状态通过不代表应用就绪。 |
