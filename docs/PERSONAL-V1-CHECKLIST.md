...
| 多模态、文件和完整恢复 | 原文来源和摘要引用已有基础；文件/PDF/图片等完整摄取、附件与数据库协调备份、恢复演练仍待完成。开发候选分支 development/personal-documents 已实现首批 UTF-8 文本文件显式导入、原文字节追溯、片段排队与字节级恢复校验（PDF/图片/OCR 明确不支持），未获独立审核批准前不合入 main。 |
...
0.14 为开发候选：增加 Claude/OpenCode 授权自动采集与客户端交付队列；未获得独立子代理批准前不算阶段验收，不合入 main。其他客户端自动采集、文件/多模态、完整恢复和真实用户部署仍未完成。
