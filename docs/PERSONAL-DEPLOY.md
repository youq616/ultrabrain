...
三个个人单元必须处于停止状态，没有排队任务。操作者先在已获准的维护窗口停止个人 target，并确认没有其他进程、自动化或管理员同时编辑/启动这些单元。失败状态、无法确定的管理器响应、陌生或手工安装的同名文件、单元别名、drop-ins、额外 `.wants`/`.requires`/`.upholds` 依赖目录以及不支持的加载路径均会拒绝变更。旧手工链接不会被自动“认领”，需要操作者单独检查和迁移。
...
只读扫描系统搜索路径时兼容 Ubuntu 的标准 `/etc/xdg/systemd/user` → `/etc/systemd/user` 映射：核对管理员归属、安全父目录、精确目标及稳定身份后检查实际目标目录。这个例外只用于该系统路径的只读检查；受管用户单元目录、导出和事务状态仍逐级禁止符号链接重定向。

systemd 的 `FragmentPath` 可以报告固定安装链接本身，也可以报告其代际文件目标。工具只接受这两种精确路径，并继续核对受管链接身份、回执和文件内容；同名路径本身不证明归属。事务完成前必须实际重载并再次校验管理器状态。
...
事务先持久化代际、回执、恢复用链接和 pending，再改变可见服务链接。可确定的普通失败会尝试恢复事务开始前状态并重载；进程崩溃、超时或补偿失败会留下恢复记录，新的写入被阻止。即使进程中断时新 current 已写入，`recover` 也恢复该事务开始前的回执。恢复过程必须完成重载后才能清除 pending，包括上次恢复已经还原链接、却在重载前再次中断的情况；不能只凭 `NeedDaemonReload=no` 判断缓存已还原。恢复前须保持个人服务停止；遇到外部修改或无法确认的状态时保留证据并拒绝继续，不删除 pending 强行解锁，也不手工拼凑链接。

## 验证范围

`test/test_personal_deploy.py` 使用临时文件系统和注入的管理器响应验证事务、冲突、崩溃及安全边界；这些单元测试不是活 systemd 验收。公开 CLI 测试覆盖参数、错误脱敏、普通账号限制与 Python 隔离。

`Personal user services` 工作流在明确获准的一次性普通用户 runner 上，先执行既有服务集成，再执行 `test/personal-deploy-integration.mjs`：实际安装、删除导出后启动、令牌认证、运行中拒绝变更、真实事务子进程中断及公开 CLI 恢复、恢复过程再次中断后的缓存重载、停止后更新、增删 Worker 配置和逐级回滚。重复恢复夹具通过持续的只读 D-Bus 单元引用防止非活动单元被回收，以稳定保留待检验的新配置缓存；它不启动单元或修改服务属性。测试检查实际管理器仍缓存新 source，而链接已恢复旧代，然后由公开恢复命令在无进一步链接变更时重载为旧 source；确认实际缓存已恢复后释放引用，再验证 HTTP。测试分别检查实际 FragmentPath、无排队任务及数据库配置保留；部署工具本身只重载，所有 start/stop 由测试夹具明确调用。新部署集成不调用模型，既有周期 Worker 集成仍使用合成供应商响应。
...
[PR #11](https://github.com/youq616/ultrabrain/pull/11) 已合入：最终候选 `8c519f23a359691a24e9780766c5baed54c58c40` 获两个独立代理 PASS，六组 PR CI 共 23 个 jobs 全部成功；push 与 PR 的实际部署集成均通过上述 15 项检查。完整提交、运行 ID、审核与首次失败证据见 [项目状态](PROJECT-STATUS.md) 和 [机器验收记录](reviews/PERSONAL-DEPLOY-EVIDENCE.json)。没有以这些 CI 代替用户实际主机、断电文件系统、真实客户端/模型或完整个人 V1 验收。
