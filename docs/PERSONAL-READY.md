...
最终应用候选 `715314236183ff75f97bc96fcfef57bd6b421192` 获两个未参与实现的独立代理 PASS；六组 PR CI 共 23 个 jobs 全部成功，普通用户下 557 Node / 438 Python 全通过、无跳过。代码树、实际 PR merge、日志哈希、首次失败和逐轮审核保存在 [验收档案](reviews/personal-ready/README.md) 与 [机器记录](reviews/personal-ready/evidence.json)。

push 与 PR 的真实用户服务 CI 均通过独立计数的 12 项就绪检查：正确实例、四种错误预期、令牌轮换、运行中源记录缺失、实际源表独占锁阻塞、释放后的同一 console 运行实例恢复，以及文件/配置/服务身份/记录数量不变。原有 15 项部署检查和 18 项服务检查也分别通过。该环境为一次性普通 Linux 账号及真实 user-systemd、PostgreSQL、认证 HTTP；新的就绪检查调用模型 0 次，既有服务检查另使用 2 次本地合成供应商响应。

本地 UID 0 环境的协议、CLI 和进程夹具结果不代替上述普通用户集成；这次验收也不代表已部署用户主机或验证完整客户端/真实模型。当前验收说明属于后续文档更新，应用 CI 归属于上述精确候选及其相同应用代码树，未在后续文档提交重新执行。
