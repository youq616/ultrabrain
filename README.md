...
`bun src/cli.mjs personal-services render` 生成全新、私有、可校验但未启用的 systemd 用户单元；默认仅本机管理台，周期整理必须另外明确授权。不会自动修改服务、配置、采集范围或模型设置。使用与验收边界见 [个人服务](docs/PERSONAL-SERVICES.md)。
